...
> _기계는 사고할 수 있을까?_ 엘런 튜링은 그의 논문 "**computing machinery and intelligence**" 에서 오늘날 튜링 테스트라고 알려진 이미테이션 게임이라는 실험을 제안했다.
...
1. 훈련 데이터셋
   ...
   - 데이터셋은 _판별자 신경망의 입력(x)_ 으로 제공된다.
2. 랜덤한 잡음 벡터
   - 랜덤한 숫자 벡터로 생성자가 *가짜 샘플 합성의 시작점*으로 사용한다.
   - _생성자 신경망의 입력(z)_ 으로 제공된다.
3. 생성자 신경망
...
5. 반복 훈련
   ...
   - 판별자의 가중치와 편향은 *분류 정확도를 최대화 하도록 업데이트*된다.
     - _x를 진짜로 판단하고 x'을 가짜로 판단하도록_ 올바른 예측의 확률을 최대화 한다.
...
1. **판별자 훈련하기**
   ...
   - _분류 오차를 계산하고 전체 오차를 역전파해서 판별자의 훈련 가능한 파라미터를 업데이트하고 분류 오차를 최소화_
2. **생성자 훈련하기**
   ...
   - 판별자 신경망을 이용해 x'을 분류
   - _분류 오차를 계산하고 역전파해서 생성자의 훈련 가능한 파라미터를 업데이트하고 판별자의 오차를 최대화_
3. **균형에 도달하기**
   ...
   - 생성자가 만드는 가짜 샘플이 이미 진짜 샘플과 구분이 안될 때 _변환하는 과정을 약간만 바꾸더라도 판별자에게 가짜 샘플과 진짜 샘플을 구별할 수 있는 실마리를 제공하고 생성자의 성능은 오히려 나빠진다_.
...
- **2단계: 잠재공간 z**
  ...
  - _잠재 공간은 보통 작은 차원으로 표현_ 되며 중간 단계의 역할을 한다.
